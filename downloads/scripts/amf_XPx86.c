/* 
   amf_XPx86.c - kernel module for anti memory forensic analysis
...
void PatchDispatcherHeaderSize(PEPROCESS ep) 
{
  ...
  DbgPrint("Patching one byte to Size in _DISPATCHER_HEADER at 0x%08x\n", dispatch_size_addr);
...
{  
  ...
  DbgPrint("Patching one byte to PoolTag at 0x%08x\n", pooltag_addr);
		   ...
		   )
{
  ...
  DbgPrint("AMF driver is unloaded\n");
  return STATUS_SUCCESS;
}
