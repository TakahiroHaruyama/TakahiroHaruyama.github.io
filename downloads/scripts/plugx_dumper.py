...
#
# This script can be used for PlugX specimens including config whose size is 0x150C/0x1510/0x1B18/0x1D18/0x2540
# How to use:
...
def parse_ver4_config(xsetting):
    ...
    result += print_config("registry hive type(=HKLM,): 0x%08X" % (hive_type))
...
def main(args):
    ...
    for table in imm.search(signature):
        if (table & 0xffff == 0) and (table == (imm.readLong(table + 0x1c) & 0xffff0000)):
            ...
            imm.log("-----------------------------------------------------")
            return "Complete.."
    imm.log("searching shellcode (getPC) ...")
    imm.getMemoryPages()
    candidate_sc = []
