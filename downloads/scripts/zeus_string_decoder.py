'''
    ZeuS 2.x Variants Deobfuscation Script
    Copyright (c) 2012 Takahiro Haruyama
'''

# You should configure the following three names
decode_structure_array = 'cci_array_struc_encoded_str'
func_getA = '?_getA@CryptedStrings@@YIXGPAD@Z'
func_getW = '?_getW@CryptedStrings@@YIXGPA_W@Z'

decode_func_names = [func_getA, func_getW]
structure_name = 'struc_encoded_str'

zeus_2_0_8_9_encoded_strings = [
...
def getSidList():
    sidlist = []
    struct_index = GetFirstStrucIdx()
    if struct_index != 0xffffffff and struct_index != -1:
        sid = GetStrucId(struct_index)
        sidlist.append(sid)
        struct_index = GetNextStrucIdx(struct_index)
        while struct_index != 0xffffffff and struct_index != -1:
            sid = GetStrucId(struct_index)
            sidlist.append(sid)
            struct_index = GetNextStrucIdx(struct_index)
    return sidlist

def getTargetSid(target_struct):
    exist_flag = False
    rsid = -1
    sidlist = getSidList()
    for sid in sidlist:
        sname = GetStrucName(sid)
...
def get_old_str(key, size, encoded_str):
    decoded_str = []
    for i in range(0, size):
        char = ((ord(encoded_str[i]) ^ key) ^ i) & 0xff
        decoded_str.append(chr(char))
    return ''.join(decoded_str)

def getAorW(key, size, ea):
    decoded_str = []
    for i in range(0, size):
        char = ((Byte(ea + i) ^ key) ^ i) & 0xff
        decoded_str.append(chr(char))
    return ''.join(decoded_str)

def main():
    sid = getTargetSid(structure_name)
    if sid < 0:
        sid = AddStrucEx(-1, structure_name, 0)
        AddStrucMember(sid, 'field_0_key', -1, (FF_WORD)&0xffffffff, -1, 2)
        AddStrucMember(sid, 'field_2_size', -1, (FF_WORD)&0xffffffff, -1, 2)
        AddStrucMember(sid, 'field_4_ptr_encoded_string', -1, (FF_DWRD)&0xffffffff, -1, 4)

    start_ea = LocByName(decode_structure_array)
    ssize = GetStrucSize(sid)
    first_off = GetMemberOffset(sid, 'field_0_key')
    second_off = GetMemberOffset(sid, 'field_2_size')
    third_off = GetMemberOffset(sid, 'field_4_ptr_encoded_string')
    ...
    while Byte(ea) != 0:
        struct_num += 1
        MakeStructEx(ea, ssize, structure_name)
        ea = ea + ssize

    decoded_ids = []
    decoded_strings = []
    for df_name in decode_func_names:
        decode_func = LocByName(df_name)
        if decode_func == BADADDR:
            print '%s: no such name' % df_name
            return
        ...
        for ref in refs:
            print '\nref=%x' % ref
            if GetMnem(ref) == 'call':
                id_related_ea = BADADDR
                decode_id = 0

                while ref != BADADDR:
                    if GetMnem(ref) == 'pop' and GetOpnd(ref, 0) == 'ecx':
                        while GetMnem(ref) != 'push':
                            ref = FindCode(ref, SEARCH_UP)
                        decode_id = GetOperandValue(ref, 0)
                        id_related_ea = ref
                        print 'decode id = %x (push/pop)' % decode_id
                        break
                    elif GetMnem(ref) == 'mov' and (GetOpnd(ref, 0) == 'cx' or GetOpnd(ref, 0) == 'ecx'):
                        decode_id = GetOperandValue(ref, 1)
                        id_related_ea = ref
                        print 'decode id = %x (mov)' % decode_id
                        break
                    elif GetMnem(ref) == 'xor' and GetOpnd(ref, 0) == 'ecx' and GetOpnd(ref, 1) == 'ecx':
                        id_related_ea = ref
                        print 'decode id = %x (xor)' % decode_id
                        break
                    ref = FindCode(ref, SEARCH_UP)
                    #print 'searching... ref=%x, %s %s %s' % (ref, GetMnem(ref), GetOpnd(ref, 0), GetOpnd(ref, 1))

                struct_ea = start_ea + ssize * decode_id
                key = Word(struct_ea + first_off)
                size = Word(struct_ea + second_off)
                ptr_encoded_string = Dword(struct_ea + third_off)
                print 'id = %x, struct_ea = %x (key = %x, size = %x, ptr_encoded_string = %x)' % (decode_id, struct_ea, key, size, ptr_encoded_string)
                decoded_string = getAorW(key, size, ptr_encoded_string)
                print decoded_string
                MakeComm(struct_ea, decoded_string)
                if id_related_ea != BADADDR:
                    MakeComm(id_related_ea, decoded_string)
                decoded_ids.append(decode_id)
                decoded_strings.append(decoded_string)
                decode_cnt += 1

        print '\n%s decodes %d strings\n'% (df_name, decode_cnt)

    print '\nlinear decoding from structure start address\n'
    decode_cnt = 0
    for i in range(0, struct_num):
        if i in decoded_ids:
            continue
        struct_ea = start_ea + ssize * i
        key = Word(struct_ea + first_off)
        size = Word(struct_ea + second_off)
        ptr_encoded_string = Dword(struct_ea + third_off)
        print 'id = %x, struct_ea = %x (key = %x, size = %x, ptr_encoded_string = %x)' % (i, struct_ea, key, size, ptr_encoded_string)
        decoded_string = getAorW(key, size, ptr_encoded_string)
        print decoded_string
        MakeComm(struct_ea, '(no xref) ' + decoded_string)
        decoded_strings.append(decoded_string)
        decode_cnt += 1
    print '\nlinear decoding decodes %d strings\n'% (decode_cnt)

    old_decoded_strings = []
    print '\ncompare with Zeus 2.0.8.9 strings...\n'
    for i in range(0, len(zeus_2_0_8_9_encoded_strings)):
        decoded_string = get_old_str(zeus_2_0_8_9_encoded_strings[i][0], zeus_2_0_8_9_encoded_strings[i][1], zeus_2_0_8_9_encoded_strings[i][2])
        #print decoded_string
        old_decoded_strings.append(decoded_string)
    print '\nRESULT: brand-new strings\n'
    for str in decoded_strings:
        if str not in old_decoded_strings:
            print str
